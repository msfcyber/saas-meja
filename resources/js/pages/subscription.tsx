...
export default function Subscription({ subscription, invoices }: Props) {
    const form = useForm({});
    const paymentError = (form.errors as unknown as Record<string, string | undefined>).payment;

    useEffect(() => {
        return router.on("flash", (event) => {
            const checkout = (event as CustomEvent).detail?.flash?.subscription_checkout as
                | { redirect_url?: string }
                | undefined;
            ...
            <div className="mx-auto w-full max-w-[1500px] flex-1 p-4 sm:p-6 lg:p-8">
                <div className="flex flex-col gap-5 sm:flex-row sm:items-end sm:justify-between">
                    <div>
                        ...
                        <p className="mt-2 max-w-2xl text-sm text-muted-foreground">
                            Kelola akses ordering, batas resource, dan pembayaran plan dari satu
                            tempat.
                        </p>
                    </div>
                ...
                <div className="mt-8 grid gap-5 xl:grid-cols-[minmax(0,1.15fr)_minmax(320px,0.85fr)]">
                    <section className="rounded-[1.5rem] border bg-[#293025] p-6 text-[#fffaf0] shadow-[0_20px_60px_-35px_rgba(41,48,37,0.8)] sm:p-8">
                        ...
                        <p className="mt-8 text-sm text-[#d8d6c8]">Plan saat ini</p>
                        <h2 className="font-display mt-1 text-3xl font-bold">
                            {plan?.name ?? "Belum ada plan"}
                        </h2>
                        {plan && (
                            <p className="mt-2 text-sm text-[#d8d6c8]">
                                {formatMoney(plan.price, plan.currency)} / {plan.billing_interval}
                            </p>
                        )}
                    ...
                    <section className="rounded-[1.5rem] border bg-card p-6 sm:p-8">
                        <div className="flex items-center gap-3">
                            ...
                            <div>
                                <h2 className="font-display text-xl font-bold">Pemakaian plan</h2>
                        ...
                        <div className="mt-7 grid gap-5">
                            {Object.entries(subscription.usage).map(([key, value]) => {
                                ...
                                const percentage =
                                    limit && limit > 0 ? Math.min((value / limit) * 100, 100) : 0;
                                    ...
                                    <div key={key}>
                                        <div className="flex items-center justify-between gap-4 text-sm">
                                            ...
                                            <span className="text-muted-foreground">
                                                {value} / {limit === null ? "tak terbatas" : limit}
                                            </span>
                                        </div>
...
Subscription.layout = { breadcrumbs: [{ title: "Subscription", href: "/subscription" }] };
